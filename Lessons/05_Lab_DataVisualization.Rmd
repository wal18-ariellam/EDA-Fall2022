...
plot1 <- ggplot(PeterPaul.chem.nutrients.surface, aes(x=daynum, y=temperature_C, color=year4))+geom_point()+ facet_wrap(PeterPaul.chem.nutrients.surface$lakename) + ylab("Temperature °C")
#2. 
...
plot2 <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C))+geom_point(aes( color=depth),size=0.5)+scale_color_viridis(option="magma", direction=-1)+ scale_x_date(date_breaks = "5 years", date_labels="%Y")+xlab("")+ylab("Temperature")
print(plot2)
...
ggplot(EPAair, aes(x=PM2.5, y=Ozone))+geom_point(aes(color=meanLat),alpha=.5)+ geom_smooth(method=lm, color="red")
...
ggplot(EPAair, aes(x=Month, y= PM2.5, color= COUNTY))+geom_bar(stat="identity") +facet_wrap(EPAair$Year)
