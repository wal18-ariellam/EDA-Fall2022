...
```{r DataExercise-1a}
high <- c(91,91,72,77,86,81,77,75,73,71)
low <- c(61,60,51,47,58,58,53,49,50,50)
...
high_c <-date_function(high)
low_c <-date_function(low)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
Temperature <- data.frame(high, low, high_c, low_c)
names(Temperature) <- c("High Temp (F)","Low Temp (F)","High Temp (C)","Low Temp (C)"); 
```
...
```{r DataExercise-1d}
summary(Temperature)
sd(Temperature$`High Temp (F)`)
summary(Temperature$`High Temp (C)`)
```
