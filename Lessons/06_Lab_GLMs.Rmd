...
```{r}
#Exercise 1:
ozoneyear <- ggplot(EPAair,aes(x= Ozone, fill=as.factor(Year)))+geom_density(alpha=0.5)+scale_fill_manual(values = c("darkgreen", "darkblue")) + labs(x= "OZone AQI value", y="Density", fill = "")
print(ozoneyear)

boxplot <- 
  ggplot(EPAair, aes(x= as.factor(Year), y= Ozone)) +geom_boxplot()+stat_summary(geom="text", fun = max, vjust=-1,size=4, label= c("b","a"))+labs(x= "", y= "Ozone AQI value")+ ylim(0,150)

print(boxplot)

violin <- 
  ggplot(EPAair, aes(x= as.factor(Year), y= Ozone)) +geom_violin(draw_quantiles = .5)+stat_summary(geom="text", fun = max, vjust=-1,size=4, label= c("b","a"))+labs(x= "", y= "Ozone AQI value")+ ylim(0,150)
...
linear_pm2.5_by_ozone <- lm(data= EPAair, PM2.5 ~ Ozone)
summary(linear_pm2.5_by_ozone )
...
pm2.5_ozone_regression <- ggplot(EPAair, aes(x=Ozone, y= PM2.5))+geom_point()+geom_smooth(method="lm", se= FALSE)
print(pm2.5_ozone_regression)
```
...
corrEPAair <- cor(EPAairx)
corrplot(corrEPAair , method = "ellipse")
corrplot.mixed(corrEPAair , upper = "ellipse")
...
AIC_pm2.5_by_ozone <- lm(data= EPAair, PM2.5 ~ Ozone + Month+ Year+meanLat+meanLng)
step(AIC_pm2.5_by_ozone)
...
new_regression <- lm(data=EPAair, PM2.5~ Ozone+Month+meanLat+meanLng)
