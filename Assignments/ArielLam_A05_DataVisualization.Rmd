...
temperature_boxplot <-
  ...
  geom_boxplot(aes(color = lakename))+ ylab("Temperature (C)")+xlab("Month")+scale_x_discrete(drop=FALSE)+ theme(legend.position="top",axis.text.x = element_text(hjust = 1))
print(temperature_boxplot)

TP_boxplot <-
  ggplot(PeterPaul, aes(x = month, y = tp_ug)) +
  geom_boxplot(aes(color = lakename))+ ylab("TP (ug)")+xlab("Month")+scale_x_discrete(drop=FALSE)+ theme(legend.position="none",axis.text.x = element_text( hjust = 1)) 
print(TP_boxplot)


TN_boxplot <-
  ggplot(PeterPaul, aes(x = month, y = tn_ug)) +
  geom_boxplot(aes(color = lakename))+ylab("TN (ug)")+xlab("Month")+scale_x_discrete(drop=FALSE)+theme(legend.position="none",axis.text.x = element_text(hjust = 1))
print(TN_boxplot)



cowplot <- plot_grid(temperature_boxplot, TP_boxplot, TN_boxplot, nrow = 3,align = 'v', rel_heights = c(12,9,9))
print(cowplot)
```
...
> Answer: The Temperatures lower and upper quartiles seem to be relatively similar through comparison of months between Paul and Peter lake with Peter Lake having the slightly lower median for the most part. The variability for Peter lake in terms of total phosphorus seems to be grater than that of Paul lake due to the range and outliers. The ranges seem to be similar for total nitrogen, however, it appears as though Peter Lake might contain larger quantities of Nitrogen based on their higher quartiles. 
