...
> Answer: AIC method suggests that all three varaibles: year4, daynum, and depth should be used to predict temperature in the multiple regression. 74.11% of the observed variance is accounted for by this model. This is only a slightly improvement over the model using only depth as the explanatory variable. 
...
```{r scatterplot.2}
#14.
graph_temp_depth <- ggplot(NTL_LTER_TempDepth, aes(x=depth, y=temperature_C))+geom_point(aes(color=lakename),alpha = 0.5)+geom_smooth(method="lm", se= FALSE)+ylim(c(0,35))+xlab("Depth (m)")+ylab("Temperature (C)")
...
```{r t.test}
crampton_ward <- NTL_LTER_TempDepth %>% filter(lakename == c("Crampton Lake", "Ward Lake" ))
...
>Answer: The test says gives a non significant p-value and therefore must accept the null hypothesis that the difference in means between the two lakes is equal to 0. The mean temperatures are pretty close to each other as the mean for Crampton Lake is 15.4 and the mean for ward lake is 14.25. The tukey test in question 16 between crampton and ward give a high p value that isn't siginicant indicating that their mean temperatures are similar and match the t-test. 
